...
public class Social_media {
	...
	public static void main(String[] args) throws Exception {
		...
		String options = "enter 0 for finish process \n"
				...
				+ "enter 16 for insert data into post table\n";
		boolean flag = true;
		int opt = 0, cnt = 0, id;
		while (flag) {
			...
			switch (opt) {
				...
				case 1:
					try {
						...
						System.out.println(
								"USER_ID | EMAIL_ID  | PHONE_NO  |  PASSWORD | FIRST_NAME | LAST_NAME | CITY | PINCODE |  DOB  | GENDER | AGE");
						while (rs.next()) {
				...
				case 3:
					try {
						...
						Emailid = inpu.next();
						((PreparedStatement) std).setString(1, Emailid);
						System.out.println("enter user 10 digit phone no ");
						phoneno = inpu.next();
						((PreparedStatement) std).setString(2, phoneno);
						System.out.println("enter user password ");
						Password = inpu.next();
						((PreparedStatement) std).setString(3, Password);
						System.out.println("enter user first name ");
						firstname = inpu.next();
						((PreparedStatement) std).setString(4, firstname);
						System.out.println("enter user last name ");
						lastname = inpu.next();
						((PreparedStatement) std).setString(5, lastname);
						System.out.println("enter user city ");
						city = inpu.next();
						((PreparedStatement) std).setString(6, city);
						System.out.println("enter user pincode ");
						pincode = inpu.nextInt();
						((PreparedStatement) std).setInt(7, pincode);
						System.out.println("enter user date of birth in yyyy-mm-dd format ");
						dob = inpu.next();
						((PreparedStatement) std).setString(8, dob);
						System.out.println("enter user gender 'M' or 'F' ");
						gender = inpu.next();
						((PreparedStatement) std).setString(9, gender);
				...
				case 8:
					try {
						...
						rs = st.executeQuery(query);
						System.out.println(
								"User_ID | Email_ID | First_name | DOB | number_of_friends | number_of_pages | number_of_posts");
						while (rs.next()) {
							String userdata = rs.getInt(1) + " | " + rs.getString(2) + " | " + rs.getString(3) + " | "
				...
				case 9:
					try {
						...
						rs = st.executeQuery(query);
						System.out.println(
								"POST_ID | USER_ID | USER_NAME | USER_AGE | POSTED_DATE | POST_CONTENT | POST_LIKES | POST_SHARES | POST_COMMENTS ");
						while (rs.next()) {
							String userdata = rs.getInt(1) + " | " + rs.getInt(2) + " | " + rs.getString(3) + " | "
				...
				case 10:
					try {
						...
						rs = st.executeQuery(query);
						System.out.println(
								"POST_ID | POST_CONTENT | POSTED_DATE | COMMENT_ID | COMMENT_CONTENT | COMMENT_DATE");
						while (rs.next()) {
							String userdata = rs.getInt(1) + " | " + rs.getString(2) + " | " + rs.getString(3) + " | "
									+ rs.getInt(4) + " | " + rs.getString(5) + " | " + rs.getString(6);
							System.out.println(userdata);
				...
				case 14:
					try {
						...
						rs = st.executeQuery(query);
						System.out
								.println("POST_ID | COMMENT_ID | COMMENT_CONTENT | COMMENT_LIKED_USER_ID | FIRST_NAME");
						while (rs.next()) {
							String userdata = rs.getInt(1) + " | " + rs.getInt(2) + " | " + rs.getString(3) + " | "
				...
				case 16:
					try {
						query = "insert into post(post_id,posted_user_id,post_date,post_content) values (?,?,DATE,?)";
						std = con.prepareStatement(query);
						String post_id, post_date, post_content;
						int posted_user_id;
						System.out.println("posted_user_id");
						posted_user_id = inpu.nextInt();
						((PreparedStatement) std).setInt(1, posted_user_id);
						System.out.println("post date");
						post_date = inpu.next();
						((PreparedStatement) std).setString(2, post_date);
						System.out.println("content of post");
						post_content = inpu.next();
						((PreparedStatement) std).setString(3, post_content);
						cnt = (int) ((PreparedStatement) std).executeUpdate();
						System.out.print(cnt + "row/s affected !!! \n");
					} catch (Exception e) {
						System.out.println(e);
